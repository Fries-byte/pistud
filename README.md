...
- **if_stmt**: to use an if statement, use ```ps.if_stmt("variable" => "value", ['pln("Hello!")'], ['pln("Bye!")'])```, this part "variable", "value", you could say if variable is value then pln("Hello!") else pln("Bye!"), else is optinal. ```ps.if_stmt(if > "variable" => is > "value", then > ['pln("Hello!")'], else > ['pln("Bye!")'])```, there are also include, which you just do this: includes => <br> <br>
